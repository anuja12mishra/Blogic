...
const Comments: React.FC<CommentsProps> = ({ blogId, authorId }) => {
    ...
    const { data: commentData, loading: commentLoading } = useFetch<{ comment: CommentData[] }>(
    ...
    const nestComments = (comments?: CommentData[]) => {
        if (!comments) return [];
        ...
        if (sortBy === 'newest') roots.sort((a,b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
        if (sortBy === 'oldest') roots.sort((a,b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
            ...
            <div className="flex justify-between items-center mb-8">
                ...
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-40 rounded-xl">
                        {(['newest', 'oldest', 'popular'] as const).map(opt => (
                            <DropdownMenuItem 
                                key={opt}
                                onClick={() => setSortBy(opt)}
                                className="flex items-center justify-between cursor-pointer"
                            >
                                <span className="capitalize">{opt === 'popular' ? 'Most Liked' : opt}</span>
                ...
                {commentLoading ? (
                    ...
                    nestedComments.map((data) => (
                        <div key={data._id}>
                            ...
                            {((data.replies && data.replies.length > 0) || replyingTo === data._id) && (
